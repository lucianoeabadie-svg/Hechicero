...
const SPELLS = {
  ...
  attack:      { name:'Bola de Fuego',   potions:{red:1,blue:1},    damage:[50,100] },
  heal:        { name:'Curación Élfica', potions:{green:1},         heal:[40,80]    },
  shield:      { name:'Escudo Arcano',   potions:{blue:1},          shield:50       },
  sawblade:    { name:'Ataque Serrucho', potions:{red:2,blue:1}, damage:'half', heal:0.20, maxUses:2 }
};
const ENEMY_CONFIGS = {
  troll: {
    name: 'TROLL OSCURO',
...
function onPointerDown(e) {
  ...
  // Mostrar botón azul central si poción es roja (para el ataque)
  if(selectedPotion==='red') showBlueDotBtn(true);
}
...
function onPointerUp(e) {
  ...
  // Si acaba de dibujar con rojo, mostrar botón azul central
  if(selectedPotion==='red' && !blueDotPressed){
    showBlueDotBtn(true);
  }
}
...
// ===================== BLUE DOT BUTTON =====================
let blueDotPressed = false;

function showBlueDotBtn(show){
  const btn = document.getElementById('blue-dot-btn');
  if(!btn) return;
  if(show && selectedPotion==='red'){
...
function pressBlueCenter(){
  if(!gameActive || G.phase!=='player') return;
  if((G.potions.blue||0)<1){ setResult('Sin pociones azules!'); return; }
  blueDotPressed = true;
  const btn = document.getElementById('blue-dot-btn');
  btn.classList.add('pressed');
  // Dibujar efecto visual del punto en el canvas
  const canvas = document.getElementById('draw-canvas');
  if(canvasCtx && canvas){
    const cx = canvas.width/2, cy = canvas.height/2;
    canvasCtx.save();
    canvasCtx.beginPath();
    canvasCtx.arc(cx, cy, 14, 0, Math.PI*2);
    canvasCtx.fillStyle = '#4080ff';
    canvasCtx.shadowColor = '#4080ff';
    canvasCtx.shadowBlur = 24;
    canvasCtx.globalAlpha = 0.95;
    canvasCtx.fill();
    // Anillo exterior
    canvasCtx.beginPath();
    canvasCtx.arc(cx, cy, 22, 0, Math.PI*2);
    canvasCtx.strokeStyle = 'rgba(100,160,255,0.5)';
    canvasCtx.lineWidth = 3;
    canvasCtx.stroke();
    canvasCtx.restore();
  }
  // Agregar trazo azul sintético como punto central
  const w = document.getElementById('draw-canvas').width;
  const h = document.getElementById('draw-canvas').height;
  G.currentDrawings.push({
    potion:'blue',
    path:[{x:w/2,y:h/2},{x:w/2+1,y:h/2+1}] // punto sintético
  });
  setResult('🔵 Punto azul activado — ahora Lanzá!');
  setTimeout(()=>{
...
function isDot(path){
  ...
  const bb=pathBBox(path);
  return (bb.w<32 && bb.h<32); // incluye el punto sintético del botón
}
...
function analyzeDrawings(drawings){
  ...
  // ── SERRUCHO: zig-zag ROJO + punto AZUL (desbloqueado y con usos) ──
  if(hasRed && hasBlue && sawbladeUnlocked() && G.sawbladeUsesLeft>0){
    const redZigzag = byPotion.red && byPotion.red.some(p=>isZigzag(p));
    if(redZigzag) return 'sawblade';
  }

  // ── BOLA DE FUEGO: círculo ROJO + punto AZUL ──
  if(hasRed && hasBlue){
    return 'attack';
  }

  // ── GOLPE ARCANO: X con ROJO solo (sin azul) ──
  if(hasRed && !hasBlue && !hasGreen){
    if(isX(byPotion.red)) return 'basic_attack';
  }

  // ── ESCUDO: cuadrado AZUL solo (sin rojo) ──
  if(hasBlue && !hasRed){
    const blueSquare = byPotion.blue.some(p=>isSquare(p));
    const blueBig    = byPotion.blue.some(p=>{ const bb=pathBBox(p); return Math.hypot(bb.w,bb.h)>35; });
    if(blueSquare) return 'shield';
    if(blueBig)    return 'shield'; // trazo azul grande aunque no sea cuadrado perfecto
    return 'shield'; // cualquier trazo azul solo → escudo
  }

  // ── CURACIÓN: triángulo VERDE solo ──
  if(hasGreen && !hasRed && !hasBlue){
    return 'heal';
  }

  // Solo verde (con otra cosa rara) → curación igual
  if(hasGreen) return 'heal';
...
function drawSpellGuide(){
  ...
  guide.height=cont.offsetHeight||120;
  ...
  if(spell==='red'){
    if(sawbladeUnlocked()){
      // Dos guías: círculo arriba + zigzag abajo
      // Círculo pequeño
      ctx.strokeStyle='rgba(255,80,80,0.55)';
      const r=Math.min(w,h)*0.2;
      ctx.beginPath(); ctx.arc(cx, cy-h*0.18, r, 0, Math.PI*2); ctx.stroke();
      ctx.font='8px Cinzel,serif'; ctx.fillStyle='rgba(255,130,80,0.6)';
      ctx.textAlign='center'; ctx.fillText('círculo=ataque',cx,cy-h*0.18+r+10);
      // Zig-zag abajo
      ctx.strokeStyle='rgba(255,160,60,0.55)';
      const peaks=3, amp=h*0.12, sx=w*0.1, ex=w*0.9, zy=cy+h*0.18;
      const segW2=(ex-sx)/peaks;
      ctx.beginPath(); ctx.moveTo(sx,zy);
      for(let i=0;i<peaks;i++){
        ctx.lineTo(sx+segW2*(i+0.5),zy-amp);
        ctx.lineTo(sx+segW2*(i+1),  zy+amp);
      }
      ctx.stroke();
      ctx.fillStyle='rgba(255,160,60,0.6)'; ctx.font='8px Cinzel,serif';
      ctx.fillText('zigzag=serrucho',cx,zy+amp+10);
    } else {
      // Círculo arriba + X abajo
      ctx.strokeStyle='rgba(255,80,80,0.65)';
      const r=Math.min(w,h)*0.2;
      ctx.beginPath(); ctx.arc(cx, cy-h*0.18, r, 0, Math.PI*2); ctx.stroke();
      ctx.font='8px Cinzel,serif'; ctx.fillStyle='rgba(255,130,80,0.6)';
      ctx.textAlign='center'; ctx.fillText('círculo=Bola de Fuego',cx,cy-h*0.18+r+10);
      // X abajo
      const xs=w*0.18, ys=h*0.12, xc=cx, yc=cy+h*0.22;
      ctx.strokeStyle='rgba(255,60,60,0.65)';
      ctx.beginPath(); ctx.moveTo(xc-xs,yc-ys); ctx.lineTo(xc+xs,yc+ys); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(xc+xs,yc-ys); ctx.lineTo(xc-xs,yc+ys); ctx.stroke();
      ctx.font='8px Cinzel,serif'; ctx.fillStyle='rgba(255,80,80,0.6)';
      ctx.fillText('X=Golpe Arcano',cx,yc+ys+12);
    }
  } else if(spell==='blue'){
    // Cuadrado fantasma (escudo) — solo azul
    ...
    // Esquinas marcadas
    [[cx-s,cy-s],[cx+s,cy-s],[cx+s,cy+s],[cx-s,cy+s]].forEach(([x,y])=>{
      ctx.beginPath(); ctx.arc(x,y,4,0,Math.PI*2);
      ctx.fillStyle='rgba(140,180,255,0.5)'; ctx.fill();
    });
    ctx.font='10px Cinzel,serif'; ctx.fillStyle='rgba(100,150,255,0.7)';
    ctx.textAlign='center'; ctx.fillText('cuadrado azul',cx,cy+s+14);
  } else if(spell==='green'){
    // Triángulo fantasma (curación)
    ctx.strokeStyle='rgba(80,210,120,0.7)';
    ...
    ctx.lineTo(cx-s,cy+s*0.6);
    ctx.closePath(); ctx.stroke();
    ctx.font='10px Cinzel,serif'; ctx.fillStyle='rgba(80,220,120,0.6)';
    ctx.textAlign='center'; ctx.fillText('triángulo →',cx,cy+s*0.6+14);
  }
  ctx.restore();
}
...
function drawRecipe(canvasId, type){
  ...
  if(type==='attack'){
    ...
    const r=24;
    ctx.beginPath(); ctx.arc(cx-8,cy,r,0,Math.PI*2); ctx.stroke();
    // Punto azul
    ctx.fillStyle='#6090ff';
    ctx.shadowColor='#6090ff';
    ctx.beginPath(); ctx.arc(cx+26,cy,6,0,Math.PI*2); ctx.fill();
    // Labels
    ctx.shadowBlur=0; ctx.font='bold 9px sans-serif'; ctx.textAlign='center';
    ctx.fillStyle='#ff8080'; ctx.fillText('🔴',cx-8,cy+r+12);
    ctx.fillStyle='#8080ff'; ctx.fillText('🔵',cx+26,cy+12);
  }
  else if(type==='heal'){
    // Triángulo verde
    ctx.strokeStyle='#50d080';
    ctx.shadowColor='#50d080'; ctx.shadowBlur=8;
    const s=28;
    ctx.beginPath();
    ...
    ctx.closePath(); ctx.stroke();
    ctx.shadowBlur=0; ctx.font='bold 9px sans-serif'; ctx.textAlign='center';
    ctx.fillStyle='#80e080'; ctx.fillText('🟢',cx,cy+s*0.55+12);
  }
  else if(type==='sawblade'){
    // Zig-zag serrucho
    ctx.strokeStyle='#ff9040';
    ctx.shadowColor='#ff9040'; ctx.shadowBlur=10;
    const peaks=3, amplitude=20, startX=10, endX=w-10, midY=cy;
    ...
    ctx.stroke();
    ctx.shadowBlur=0; ctx.font='bold 9px sans-serif'; ctx.textAlign='center';
    ctx.fillStyle='#ff9040'; ctx.fillText('🔴×2 + 🔵×1',cx,cy+amplitude+16);
  }
  else if(type==='shield'){
    // Cuadrado azul solo
    ...
    [[cx-s,cy-s],[cx+s,cy-s],[cx+s,cy+s],[cx-s,cy+s]].forEach(([x,y])=>{
      ctx.beginPath(); ctx.arc(x,y,4,0,Math.PI*2);
      ctx.fillStyle='#80a0ff'; ctx.fill();
    });
    ctx.shadowBlur=0; ctx.font='bold 9px sans-serif'; ctx.textAlign='center';
    ctx.fillStyle='#8080ff'; ctx.fillText('🔵 solo azul',cx,cy+s+13);
  }
}
...
function selectPotion(p){
  ...
  // Ocultar botón azul si cambian a otro color
  if(p!=='red') showBlueDotBtn(false);
  requestAnimationFrame(drawSpellGuide);
}
